...
rank <- rbind(TMM_rank, old_rank)

ggplot(rank, aes(x = effect, y = t, group = method,
    label = t, color = method)) +
...
```{r}
dge <-  DGEList(counts = counts(sim))
dge <- edgeR::calcNormFactors(dge)
nc <- cpm(dge, normalized.lib.sizes = TRUE, log = FALSE)
```
