...
devDF <- do.call(rbind, devDF)
```

```{r violin, fig.height=8, fig.width=10}
ggplot(devDF, aes(x = factor(chip), y = logFC, fill = method)) +
...
```{r boxplot2, fig.height=8, fig.width=10}
ggplot(devDF, aes(x = factor(method), y = logFC, fill = chip)) +
  geom_violin(alpha = 0.5) +
  geom_boxplot(width = 0.2, position = position_dodge(0.9), alpha = 0.4) +
  scale_fill_brewer(palette = "Blues") +
  facet_wrap(~ effect, ncol = 3, labeller = \(.) label_both(.), scales="free") +
  xlab("New or old method")
```
